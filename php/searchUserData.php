...
$sql = "SELECT * FROM user WHERE id!=$currentUserId AND isDeleted=0 AND (user.userName LIKE '%$userInfo%' OR user.name LIKE '%$userInfo%')";
$result = $connection -> query($sql);
if($result -> num_rows > 0){
    ?>
    <div class='container alert alert-success text-center' role='alert'>以下是用户名或真实姓名带有 <? echo $userInfo ?> 的用户信息</div>
